...
$theTASK     = ($HashOfTaskC | Out-Gridview -Title "DELETE SOME DIRS" -PassThru)
...
if ($HashOfTaskC.Contains($theTASK.Name)) {    # The Task
    ...
    if ((Test-Path -Path "$theTASK.Value") -eq $false) {
        ...
        Write-Host "Cannot find path "$theTASK.Value" because it does not exist.  LoL!" -ForegroundColor Cyan #Yellow  #Blue
    ...
    if ((Test-Path -Path "$theTASK.Value") -eq $true) {
        ...
        Write-Host " "
        Write-Host "Test-Path the path" $theTASK.Value
