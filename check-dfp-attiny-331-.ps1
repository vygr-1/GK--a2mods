...
<#    # 'dfp\zip',  dfp\zip\Atmel.ATtiny_DFP.2.0.368.atpack , ...


#  if    'dfp\zip' dir not available,
#    or  'dfp\zip' dir is available
#      but not contain 'Atmel.ATtiny_DFP.2.0.368.atpack'
...
#    Searching for 'Atmel.ATtiny_DFP.2.0.368.atpack',
...
<###   extracting the pack 'Atmel.ATtiny_DFP.2.0.368.atpack'
...
###   ###   ###   ###   ###   ###   ###   
    ...
    Write-Host " "
    Write-Host " "
    Write-Host "    The 'dfp\attiny' directory (`$attiny):" $attiny
...
### IF 'Atmel.ATtiny_DFP.2.0.368.atpack' NOT available
### IF 'Atmel.ATtiny_DFP.2.0.368.atpack' NOT available
### IF 'Atmel.ATtiny_DFP.2.0.368.atpack' NOT available
...
### IF 'Atmel.ATtiny_DFP.2.0.368.atpack' is available
### IF 'Atmel.ATtiny_DFP.2.0.368.atpack' is available
### IF 'Atmel.ATtiny_DFP.2.0.368.atpack' is available
...
if (($testATtPack -eq $true) -and ($attPackSize -gt 31457000)) {
    ...
    Write-Host " "
    Write-Host "Recheck the attiny dir ...  "
    Write-Host "The 'dfp\attiny' directory (`$attiny):" $attiny
...
#   $attiny           = "$PSScriptRoot\arch\dfp\attiny"
    ...
    if (($testATTINY -eq $true) -and ($attinyDirSize -gt 109012000)) {
        ...
        Write-Host "attiny dir size  : $attinyDirSize bytes ($attinyDirSizeMB MB)" -ForegroundColor Yellow
        Write-Host " "
        Write-Host "dfpStatus        :" $dfpStatus
...
#   $FindingAvrDD   = gci -Path $RootDrive\ -Filter "Atmel.ATtiny_DFP.2.0.368.atpack" -recurse 
#   $FindingavrDD   = gci -Path $RootDrive\ -Filter "Atmel.ATtiny_DFP.2.0.368.atpack" -recurse 
#   $getavrDD   = gci -Path $RootDrive\ -Filter "Atmel.ATtiny_DFP.2.0.368.atpack" -recurse 
#   $getavrDD   = gci -Path $RootDrive\ -Filter "Atmel.ATtiny_DFP.2.0.368.atpack" -recurse 

#   $avrddPack    = gci -Path $RootDrive\ -Filter "Atmel.AVR-Dx_DFP.2.2.253.atpack" -recurse 
...
# $FindingAttiny   = gci -Path $RootDrive\ -Filter "Atmel.ATtiny_DFP.2.0.368.atpack" -recurse 
